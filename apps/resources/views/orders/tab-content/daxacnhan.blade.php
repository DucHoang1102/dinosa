<div role="tabpanel" class="tab-pane" id="daxacnhan"><!--Content Tab Đơn mới -->	
	...
	<div class="table-tbody">
		<table class="table table-hover">
			<tbody>
				@if (count($orders_daXacNhan) == 0){!! '<div style="font-style:italic;font-weight:bold;padding-left:10px">Dữ liệu trống</div>' !!}@endif
				...
				<tr>
					...
					@foreach ($order->products as $product)
						<div class="product-success bg-{{ $product->status }}" status={{ $product->status }} id="{{ $product->id }}" url-image="{{ $product->url_image->src_f_a3 }}"><span class="glyphicon glyphicon-print" aria-hidden="true"></span> {{ $product->name }} </div>
					@endforeach
					</td>
