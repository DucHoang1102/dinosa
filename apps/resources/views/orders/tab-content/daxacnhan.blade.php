			...
			<tbody>
				...
				<tr>
					...
					@foreach ($order->products as $product)
						<div class="product-success bg-{{ $product->status }}" id="{{ $product->id }}" url-image="{{ $product->url_image->src_f_a3 }}"><span class="glyphicon glyphicon-print" aria-hidden="true"></span> {{ $product->name }} </div>
					@endforeach
					</td>
